...
class CompassSensorService:
    ...
    def start_listening(self) -> CompassDiagnostics:
        ...
        try:
            ...
            try:
                self._sensor_manager.registerListener(callback, self._magnetic_sensor, _SENSOR_DELAY_UI)
                self._sensor_manager.registerListener(callback, self._accel_sensor, _SENSOR_DELAY_UI)
            except Exception as exc:
        ...
        class _Impl(PythonJavaClass):
            ...
            def onSensorChanged(self, event):
                ...
                try:
                    ...
                    log(f"compass: attach_thread() failed in onSensorChanged: {exc!r}")
                    return
                try:
                    ...
                    # A single bad reading must never crash the app --
                    # logged once in a while would be nice, but a
                    # per-event try/except here is itself on the hot
                    # path, so keep it silent after the fact; the
                    # broader failure modes are already caught upstream
                    # in probe()/start_listening().
                    del exc
