...
_DESTINATION_PATTERNS: tuple[re.Pattern, ...] = (
    ...
    re.compile(r"рухається\s+на\s+([А-ЯҐЄІЇа-яґєії'\-]+)", re.IGNORECASE),
    re.compile(r"на\s+([А-ЯҐЄІЇа-яґєії'\-]+)", re.IGNORECASE),
    # Bare "до <місто>" ("to <city>") -- every candidate still has to
    # resolve via resolve_city() below to be accepted, so this common
    # word being ambiguous on its own ("until", "to") is harmless: it
    # only ever contributes a place if what follows is a real city name.
    re.compile(r"до\s+([А-ЯҐЄІЇа-яґєії'\-]+)", re.IGNORECASE),
