...
class LoggerService:
    ...
    def history(self, message: str) -> None:
        """Record a permanent threat-lifecycle entry (appeared/moved/
        destroyed/all-clear).

        Unlike debug/info/warning/error, these are never dropped by the
        rolling diagnostic-log prune below -- only a very generous
        overall safety cap (``DEFAULTS.MAX_HISTORY_ENTRIES``) applies,
        so "when did this threat appear, move, and get destroyed"
        remains answerable for as long as the app has been running,
        instead of scrolling off alongside routine diagnostic noise.
        """
        self._log(LogLevel.HISTORY, message)
    ...
    def _log(self, level: LogLevel, message: str) -> None:
        ...
        # HISTORY entries (permanent threat lifecycle) and everything else
        # (rolling diagnostic log) are pruned independently, each against
        # its own cap -- so a burst of ordinary DEBUG/INFO chatter can
        # never push a threat-destroyed record out of the journal, and
        # vice versa a long run of history entries can't suppress recent
        # diagnostics either.
        diagnostic_entries = [e for e in self._entries if e.level != LogLevel.HISTORY]
        history_entries = [e for e in self._entries if e.level == LogLevel.HISTORY]
        if len(diagnostic_entries) > DEFAULTS.MAX_LOG_ENTRIES:
            diagnostic_entries = diagnostic_entries[-DEFAULTS.MAX_LOG_ENTRIES :]
        if len(history_entries) > DEFAULTS.MAX_HISTORY_ENTRIES:
            history_entries = history_entries[-DEFAULTS.MAX_HISTORY_ENTRIES :]
        if len(diagnostic_entries) + len(history_entries) != len(self._entries):
            merged = diagnostic_entries + history_entries
            merged.sort(key=lambda e: e.timestamp)
            self._entries = merged

        try:
            log_level_name = level.value if level != LogLevel.HISTORY else "INFO"
            _stdlib_logger.log(getattr(logging, log_level_name), message)
        except Exception:
            pass
