...
class AlertService:
    ...
    def apply_nationwide_alert(self, is_active: bool) -> int:
        """Set EVERY region's active state at once (a genuinely nationwide
        alert/cancellation, e.g. "Вся Україна ракетна небезпека", a
        MiG-31K/Kinzhal launch, or "Відбій по всій Україні" -- see
        ``app.services.region_alert_parser.parse_nationwide_alert``).

        Same per-region mutation ``set_region_active`` already does,
        just applied to all 27 regions in one pass with a single
        ``_notify()`` at the end instead of 27 separate ones. Returns
        how many regions actually changed state, so a caller can decide
        whether it's worth logging/making noise about.
        """
        changed_count = 0
        now = datetime.now()
        for state in self._region_states.values():
            if state.is_active != is_active:
                changed_count += 1
            state.is_active = is_active
            state.risk_level = RiskLevel.HIGH if is_active else RiskLevel.NONE
