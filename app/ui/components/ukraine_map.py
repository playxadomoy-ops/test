...
class UkraineMap(ft.Container):
    ...
    def _build_settlement_shapes(self, region: Region) -> list[canvas.Shape]:
        """Cities/towns/villages inside a focused oblast, with label-overlap reduction."""
        shapes: list[canvas.Shape] = []
        ...
        # Priority (raion-center) settlements are placed first so they
        # keep their label even when smaller neighbors have to give
        # theirs up -- every settlement's dot is always drawn regardless.
        ordered_names = sorted(settlements.keys(), key=lambda n: (n not in priority_names, n))
