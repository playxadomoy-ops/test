...
from app.ui.components.label_layout import place_labels, scaled_icon_factor, scaled_settlement_sizes
...
#: produced unmodified by app.services.movement_parser -- nothing about
#: parsing changes here) that distinguish a MISSILE-type movement's
#: sub-icon. Checked in this order; ballistic-specific terms win over the
#: bare generic "ракет" stem so e.g. "балістична ракета" (which matches
#: both "балістик" and "ракет") correctly renders as ballistic, not cruise.
_BALLISTIC_KEYWORDS = frozenset({"балістик", "кинджал", "іскандер"})
_CRUISE_KEYWORDS = frozenset({"калібр", "х-101", "х-555"})


def _missile_subtype(movement: ThreatMovement) -> str:
    """Return "ballistic" or "cruise" for a MISSILE-type movement.

    Purely a view-layer classification for icon selection -- reads only
    ``matched_keywords`` (already produced, unchanged, by
    ``movement_parser.py``); never re-parses ``movement.text`` and never
    changes ``ThreatType`` or any stored/persisted data. A bare "ракет"
    match with no more specific term present defaults to "cruise" (the
    more common case for an unqualified missile mention).
    """
...
_ICON_STROKE_COLOR = theme.BACKGROUND
_ICON_STROKE_WIDTH = 1.2
...
def _rotate(point: tuple[float, float], angle: float) -> tuple[float, float]:
    """Rotate a local-space point by ``angle`` radians about the origin."""
    x, y = point
...
def _icon_shapes(
    ...
    """Build the rotated, colored vector icon (+ track ring, + group
    badge) for one movement.

    ``angle`` is in the same atan2(dy, dx) convention as the arrow
    bearing in ``_arrow_shapes`` -- 0 points along +x (screen-right).
    ``scale_factor`` (see ``label_layout.scaled_icon_factor``) keeps the
    icon a consistent, legible size across phone and desktop render
    widths instead of a fixed pixel size that looked right only on the
    viewport it was originally tuned against.
    ...
    """
    color, rings = _icon_geometry(movement)
    track_radius = _ICON_TRACK_RING_RADIUS * scale_factor
    stroke_width = _ICON_STROKE_WIDTH * scale_factor
    ...
    for ring in rings:
        ...
        transformed = [_rotate((px * scale_factor, py * scale_factor), angle) for px, py in ring]
        x0, y0 = transformed[0]
        elements.append(canvas.Path.MoveTo(x + x0, y + y0))
        for lx, ly in transformed[1:]:
            elements.append(canvas.Path.LineTo(x + lx, y + ly))
        elements.append(canvas.Path.Close())
        shapes.append(canvas.Path(elements, paint=ft.Paint(color=color, style=ft.PaintingStyle.FILL)))
        shapes.append(
            canvas.Path(
                elements,
                paint=ft.Paint(color=_ICON_STROKE_COLOR, style=ft.PaintingStyle.STROKE, stroke_width=stroke_width),
            )
        )
...
class MovementMap(ft.Container):
    ...
    def __init__(
        ...
        #: see ``scaled_settlement_sizes()`` (label_layout.py) for why
        #: these aren't fixed constants.
        self._settlement_dot_radius: float = 2.2
        self._settlement_label_size: float = 9.0
        #: Same reasoning as the settlement attrs above, but for movement
        #: icons/arrows -- see ``label_layout.scaled_icon_factor``.
    ...
    def _build_static_shapes(self) -> list[canvas.Shape]:
        ...
        #
        # Label placement uses app/ui/components/label_layout.py's real
        # collision-avoidance algorithm (see its module docstring for why
        # a fixed anchor-to-anchor pixel distance used to cause labels to
        # overlap on narrower viewports, most visibly on real Android
        # phones): every settlement's dot is always drawn, and its label
        # is repositioned rather than hidden if it would collide.
        for region in self._selected_regions:
            ...
            for placed in place_labels(anchors, self._settlement_label_size, priority_names):
                ...
                shapes.append(
                    canvas.Text(
                        ...
                        style=ft.TextStyle(size=self._settlement_label_size, color=_SETTLEMENT_DOT_COLOR),
                    )
                )
    ...
    def _handle_map_tap_up(self, e: ft.TapEvent) -> None:
        """Resolve a tap to a movement icon first, else fall back to a Region.
        ...
        icon is close enough, the tap is resolved as a region tap
        exactly as before.
        """
