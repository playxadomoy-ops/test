...
#: Historical note: this module used to give Kyiv Oblast/Cherkasy an
#: extra-tight crop margin (1.5% vs. the standard 10% below) for
#: permanent settlement dots/labels that have since been removed
#: entirely (see this module's changelog note near
#: _build_static_shapes). That tight margin turned out to be a real bug,
#: not just stale tuning: with almost no margin, a tap even a couple
#: pixels from the rendered card's edge falls outside Kyiv's own
#: (irregular, non-rectangular) polygon and into whichever neighboring
#: oblast's territory happens to poke into that corner/edge -- verified
#: this is exactly what caused "clicking near Kyiv Oblast's border opens
#: Chernihiv/Cherkasy/Zhytomyr/Vinnytsia instead". Every selection now
#: uses the same standard margin.
...
#: Sized to comfortably cover the LARGEST rendered icon (Shahed, at
#: _ICON_SIZE_MULTIPLIER's 1.18x -- see _icon_image_control), not just
#: the shared base icon size; a smaller tap radius than the icon's own
#: visual size means a tap right on the edge of a big icon can silently
#: miss it.
_ICON_TAP_RADIUS = 24.0
...
#: Per-threat-type size multiplier on top of the shared base icon size
#: below -- Shahed noticeably larger (it's the most numerous, most
#: reported threat type and was "too small" before), cruise missile
#: slightly smaller, ballistic/aircraft at the shared baseline (the
#: ballistic silhouette itself is already visually thicker/heavier, see
#: tools/generate_icons.py, so it doesn't also need a larger bounding size).
_ICON_SIZE_MULTIPLIER: dict[ThreatType, float] = {
    ThreatType.SHAHED: 1.18,
    ThreatType.UAV: 1.18,
    ThreatType.MISSILE: 1.0,  # ballistic/cruise distinguished by _missile_subtype below
    ThreatType.AIRCRAFT: 1.0,
    ThreatType.UNKNOWN: 1.0,
}
_CRUISE_MISSILE_SIZE_MULTIPLIER = 0.9


def _icon_image_control(
    movement: ThreatMovement, x: float, y: float, angle: float, scale_factor: float = 1.0,
) -> ft.Control:
    """Build the real SVG threat-type icon as a positioned, rotated ``ft.Image``.

    ``angle`` is the same atan2(dy, dx) convention as the arrow bearing
    in ``_arrow_shapes`` (0 = pointing along +x/screen-right); the
    source SVGs are drawn nose-right at angle 0 (see
    ``tools/generate_icons.py``), so ``ft.Rotate(angle)`` lines the
    glyph up with the movement's actual reported direction directly,
    with no extra offset.
    """
    multiplier = _ICON_SIZE_MULTIPLIER.get(movement.threat_type, 1.0)
    if movement.threat_type is ThreatType.MISSILE and _missile_subtype(movement) != "ballistic":
        multiplier = _CRUISE_MISSILE_SIZE_MULTIPLIER
    size = 2 * _ICON_TRACK_RING_RADIUS * scale_factor * 1.02 * multiplier
    return ft.Container(
        left=x - size / 2,
        top=y - size / 2,
        width=size,
...
class MovementMap(ft.Container):
    ...
    def _compute_layout(self) -> None:
        ...
        margin_x = box_w * _CROP_MARGIN_RATIO
        margin_y = box_h * _CROP_MARGIN_RATIO
        min_x -= margin_x
        min_y -= margin_y
        box_w += 2 * margin_x
        box_h += 2 * margin_y
        ...
        # tall/narrow ones.
        scale = min(available_width / box_w, _MAX_RENDER_HEIGHT / box_h)
    ...
    def _build_map_and_arrow_shapes(self) -> list[canvas.Shape]:
        ...
        if len(anchors) > 1:
            min_separation = 2 * _ICON_TRACK_RING_RADIUS * self._icon_scale_factor * 1.05
    ...
    def _handle_map_tap_up(self, e: ft.TapEvent) -> None:
        ...
        not a second, separately-maintained implementation), else a
        plain oblast tap.

        When a selection is active (``self._selected_regions`` non-empty,
        i.e. this map is cropped/zoomed to specific oblast(s)), a tap
        that resolves to a region OUTSIDE that selection is silently
        ignored -- no callback fires, and there is no "nearest selected
        region" fallback. A cropped card's rendered canvas always
        includes a margin around the selection's bounding box (see
        ``_compute_layout``), and since real administrative borders are
        irregular, that margin can visibly contain a sliver of a
        neighboring, UNselected oblast's actual territory -- without
        this check, tapping there would open that neighbor even though
        it was never selected. This is the actual hit-testing gate, not
        just a visual crop: unselected regions never receive tap events
        on this map, regardless of what's technically visible/rendered
        near the edge.
        """
        ...
        # Hit-testing gate: with an active selection, a resolved region
        # outside it is not clickable at all -- ignored outright, never
        # falls back to any other region.
        if self._selected_regions and region not in self._selected_regions:
            return
