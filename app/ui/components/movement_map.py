...
def _icon_geometry(movement: ThreatMovement) -> tuple[str, list[list[tuple[float, float]]]]:
    ...
    ``_icon_asset_name`` and for sizing math -- the actual on-screen
    glyph is the real PNG asset from ``assets/icons/`` (see
    ``_build_icon_image_controls``), not this vector path.
    """
...
#: Real transparent PNG icon assets (see ``tools/generate_icons.py``,
#: which rasterizes this same module's ``_ICON_RING_*``/``_ICON_RINGS_*``
#: vector geometry), one per threat category -- served via ``assets_dir``
#: (see ``main.py``'s ``ft.app(...)`` call) and referenced here by their
#: exact filename. Every value in this mapping has a corresponding file
#: actually present under ``assets/icons/`` (verified in this project's
#: test/regression pass) -- there is no threat type that falls through
#: to a missing asset reference.
_ICON_ASSET_UAV = "icons/shahed_uav.png"
_ICON_ASSET_CRUISE = "icons/cruise_missile.png"
_ICON_ASSET_BALLISTIC = "icons/ballistic_missile.png"
_ICON_ASSET_AIRCRAFT = "icons/aircraft.png"


def _icon_asset_name(movement: ThreatMovement) -> str:
    """Return the real PNG asset path for a movement's threat-type icon.

    Same category logic as ``_icon_geometry`` above (kept in sync
    deliberately -- both read ``movement.threat_type``/
    ``_missile_subtype`` the same way), just returning an asset path
    instead of a vector color+rings pair. ``ThreatType.UNKNOWN`` (no
    specific weapon named) uses the UAV silhouette as the most neutral
    generic "unidentified aerial threat" glyph -- not a missing/blank icon.
    """
    if movement.threat_type in (ThreatType.SHAHED, ThreatType.UAV):
        return _ICON_ASSET_UAV
    if movement.threat_type is ThreatType.AIRCRAFT:
        return _ICON_ASSET_AIRCRAFT
    if movement.threat_type is ThreatType.MISSILE:
        if _missile_subtype(movement) == "ballistic":
            return _ICON_ASSET_BALLISTIC
        return _ICON_ASSET_CRUISE
    return _ICON_ASSET_UAV
