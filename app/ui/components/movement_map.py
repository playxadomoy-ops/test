...
def _icon_shapes(movement: ThreatMovement, x: float, y: float, angle: float) -> list[canvas.Shape]:
    """Build the rotated, colored vector icon (+ track ring) for one movement.
    ...
    for ring in rings:
        ...
        transformed = [_rotate(p, angle) for p in ring]
        x0, y0 = transformed[0]
        ...
        shapes.append(
            canvas.Path(
                elements,
                paint=ft.Paint(color=_ICON_STROKE_COLOR, style=ft.PaintingStyle.STROKE, stroke_width=_ICON_STROKE_WIDTH),
            )
        )
    return shapes
    ...
    def _build_map_and_arrow_shapes(self) -> list[canvas.Shape]:
        ...
        for movement in self._movements:
            if movement.has_direction:
                ...
                shapes.extend(_icon_shapes(movement, mid_x, mid_y, angle))
                self._movement_hit_points.append((mid_x, mid_y, movement))
                ...
                shapes.extend(_icon_shapes(movement, px, py, _ICON_DEFAULT_ANGLE))
                self._movement_hit_points.append((px, py, movement))
