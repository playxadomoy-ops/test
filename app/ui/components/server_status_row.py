...
_STATUS_STYLE: dict[str, tuple[str, str]] = {
    "connecting": ("\U0001F7E1", _COLOR_SEARCHING),
    "reconnecting": ("\U0001F7E1", _COLOR_SEARCHING),
    "waiting_approval": ("\U0001F7E0", _COLOR_WAITING),
    "connected": ("\U0001F7E2", _COLOR_CONNECTED),
    "unreachable": ("\U0001F534", _COLOR_ERROR),
...
class ServerStatusRow(ft.Container):
    ...
    def set_registration_status(self, status_code: str, message: str) -> None:
        ...
        "waiting_approval", "connected", "unreachable", "rejected" -- an
        unrecognized code falls back to the "unreachable" look rather
        than raising, so a future new status from ServerClient degrades
        gracefully instead of crashing the Dashboard.
        """
        emoji, color = _STATUS_STYLE.get(status_code, _STATUS_STYLE["unreachable"])
