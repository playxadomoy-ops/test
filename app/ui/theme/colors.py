...
# One consistent scale instead of ad-hoc numbers (20/14/10/8/...) scattered
# per file -- new/updated cards pick one of these rather than a bespoke
# value, so the whole app reads as one coherent design system.
RADIUS_LG = 20   # top-level cards (dashboard sections, dialogs)
RADIUS_MD = 14   # nested rows/tiles inside a card (channel row, stat tile)
RADIUS_SM = 10   # small chips/pills/message bubbles
RADIUS_XS = 6    # tiny inline elements (badges, tag pills)

# --- Shared animation timing --------------------------------------------
# One consistent easing feel across the whole app (cards, dialogs, map
# zoom) instead of each file picking its own duration/curve -- this is
# what makes transitions read as "one product" rather than a patchwork.
ANIM_CURVE = ft.AnimationCurve.EASE_OUT_CUBIC
ANIM_FAST = ft.Animation(160, ANIM_CURVE)
ANIM_NORMAL = ft.Animation(260, ANIM_CURVE)
ANIM_SLOW = ft.Animation(380, ANIM_CURVE)

BLACK = "#000000"


def elevation_shadow(color: str = BLACK, opacity: float = 0.35, blur: float = 18.0) -> ft.BoxShadow:
    """A soft, consistent drop shadow used to give cards a sense of depth.

    Centralized so every "elevated" card in the app (dashboard sections,
    dialogs, the settings form) uses the exact same shadow shape --
    tuning it once here changes it everywhere instead of hunting down
    per-file BoxShadow literals.
    """
    return ft.BoxShadow(
        spread_radius=0,
        blur_radius=blur,
