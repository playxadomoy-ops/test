...
Layout mirrors the project's reference design from top to bottom:
  ...
  2. a tappable "сервер" quick-access row (connection status to the
     centralized backend -- see app/services/server_client.py),
  3. the real geographic Ukraine map,
  4. (below the fold, scrollable) the stats grid and a live feed of the
     most recently processed threat events -- kept for functional
...
from app.ui.components.server_status_row import ServerStatusRow
from app.ui.components.stats_panel import StatsPanel
from app.ui.components.threat_card import ThreatCard
from app.ui.components.ukraine_map import OnDistrictTapped, OnRegionTapped, UkraineMap
...
class OverviewView(ft.Column):
    """The main dashboard: alert status, server connection, map, stats, and feed."""

    def __init__(
        self,
        on_region_tap: Optional[OnRegionTapped] = None,
        on_district_tap: Optional[OnDistrictTapped] = None,
        on_open_server_settings: Optional[Callable[[], None]] = None,
    ) -> None:
        """Build the full dashboard shell."""
        self.threat_card = ThreatCard()
        self.server_status_row = ServerStatusRow(on_tap=on_open_server_settings)
        self.ukraine_map = UkraineMap(on_region_tap=on_region_tap, on_district_tap=on_district_tap)
        ...
        self._messages_empty = ft.Row(
            ...
            controls=[
                ...
                ft.Text(
                    "Повідомлень ще немає. Вони з'являться тут, щойно сервер "
                    "почне надсилати оброблені події.",
                    size=12,
    ...
    def set_server_status(self, connected: bool, configured: bool) -> None:
        """Refresh the "сервер" quick row's connection indicator."""
        self.server_status_row.set_status(connected, configured)

    def set_server_stats(self, stats: dict) -> None:
        """Refresh the "сервер" quick row's live stats line."""
        self.server_status_row.set_stats(stats)
