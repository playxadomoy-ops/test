...
class SettingsView(ft.Column):
    ...
    def __init__(
        ...
        # --- Air Alert Analyzer SERVER (centralized backend) -- entirely
        # separate credentials from the Telegram api_id/api_hash above.
        # This device only ever sends its own device_id/token here, and
        # the server never returns Telegram API id/hash/phone/session in
        ...
        sections.append(
            self._section(
                ...
                [
                    ft.Text(
                        "Підключення до вашого власного сервера Air Alert Analyzer -- окреме "
                        "джерело даних від прямого моніторингу Telegram вище. Дані для входу "
                        "(Device ID / Token) видає адміністратор сервера в його "
                        "розділі «Користувачі».",
                        size=11,
