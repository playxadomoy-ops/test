...
_SERVER_STATUS_STYLE: dict[str, tuple[str, str]] = {
    "connecting": ("\U0001F7E1", theme.TEXT_SECONDARY),
    "reconnecting": ("\U0001F7E1", theme.TEXT_SECONDARY),
    "waiting_approval": ("\U0001F7E0", "#F59E0B"),
    "connected": ("\U0001F7E2", "#22C55E"),
    "unreachable": ("\U0001F534", "#EF4444"),
    "rejected": ("\U0001F534", "#EF4444"),
}


#: status_code -> the short, non-technical subtitle shown under the
#: status line -- exactly the wording requested for the two states a
#: normal user actually needs to distinguish ("it's fine" vs "it's
#: retrying on its own"); the two in-between states get their own
#: short reassurance rather than falling back to either of those.
_SERVER_SUBTITLE: dict[str, str] = {
    "connecting": "Повторне підключення автоматично...",
    "reconnecting": "Повторне підключення автоматично...",
    "unreachable": "Повторне підключення автоматично...",
    "connected": "Автоматичне підключення активне.",
    "waiting_approval": "Очікуйте підтвердження адміністратора сервера.",
    "rejected": "Зверніться до адміністратора сервера.",
}
...
class SettingsView(ft.Column):
    ...
    def __init__(
        ...
        self._server_status_text = ft.Text(
            "Підключення...", size=13, weight=ft.FontWeight.W_600, color=theme.TEXT_SECONDARY
        )
        #: Short, non-technical second line under the status -- exactly
        #: what a normal user (not a developer) needs to know: either
        #: "it's working" or "it's trying again on its own", nothing
        #: about IPs, tokens, or retries/backoff. Updated alongside the
        #: status line in set_server_status() below.
        self._server_subtitle_text = ft.Text(
            "Підключення відбувається автоматично.", size=11, color=theme.TEXT_MUTED
        )
    ...
    def set_server_status(self, status_code: str, message: str) -> None:
        ...
        ``status_code`` is one of "connecting", "reconnecting",
        "waiting_approval", "connected", "unreachable", "rejected" -- see
        app/services/server_client.py's OnStatusChanged. Purely
        read-only display; there is nothing here for the user to act on.
        """
        emoji, color = _SERVER_STATUS_STYLE.get(status_code, _SERVER_STATUS_STYLE["unreachable"])
        self._server_icon_text.value = emoji
        self._server_status_text.value = message
        self._server_status_text.color = color
        self._server_subtitle_text.value = _SERVER_SUBTITLE.get(
            status_code, "Повторне підключення автоматично..."
        )
        if self.page is not None:
            self._server_icon_text.update()
            self._server_status_text.update()
            self._server_subtitle_text.update()
