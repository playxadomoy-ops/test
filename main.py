...
async def main(page: ft.Page) -> None:
    ...
    try:
        ...
        # Single shared Audio control for the region-alert notification --
        # one instance means one playback channel: calling .play() again
        # while it's still playing restarts that same channel rather than
        # starting a second overlapping stream, which is exactly what
        # "do not overlap multiple sounds" needs, for free, without any
        # extra bookkeeping. ft.Audio ships with Flet itself (no new
        # dependency) and works via a small platform plugin on Android,
        # Windows, Linux, and macOS alike.
        alert_audio = ft.Audio(src="alert_notification.wav")
        ...
        def on_region_alert_triggered(regions: list[Region]) -> None:
            ...
            try:
                ...
                logger.info(f"Сповіщення про тривогу відтворено для: {names}")
            except Exception as exc:  # noqa: BLE001 - never let a notification hiccup break polling
                logger.error(f"Не вдалося відтворити сповіщення про тривогу: {exc}")
