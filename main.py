...
async def main(page: ft.Page) -> None:
    ...
    try:
        ...
        #: (channel_username, normalized-text fingerprint) -> when it was
        #: last processed. Anti-spam guard against the exact same message
        #: being handled twice (a channel/library redelivering an update,
        #: a destruction report echoed verbatim by more than one listener
        #: path, or literally the same report repeated by a channel) --
        #: same 10-minute window as movement continuation
        #: (movement_parser.CONTINUATION_MAX_GAP_SECONDS), so "how long is
        #: an identical report suppressed before counting as a new event"
        #: is one consistent answer across the app, not two different
        #: windows for what's conceptually the same rule. A genuinely new
        #: real-world event reusing identical wording MORE than 10
        #: minutes later is accepted as new, exactly like a route report.
        _recent_message_fingerprints: dict[tuple[str, str], datetime] = {}
        _DUPLICATE_MESSAGE_WINDOW_SECONDS = 600.0
        ...
        overview_view = OverviewView(
            ...
            on_open_server_settings=handle_open_channels,
        )
        ...
        #
        # Mirrors _process_telegram_message's role, but for the SEPARATE
        # server data source (see app/services/server_client.py): the
        # server already ran the full parse/dedup/continuation pipeline
        # server-side, so these handlers only need to translate its JSON
        # payload into the SAME ThreatMovement objects the Telegram path
        # produces, and feed them into the SAME `movements` list /
        # `_refresh_movement_views()` -- one Dashboard, fed by either or
        # both sources, exactly like alerts.in.ua vs Telegram parsing for
        # region-level status already works in AlertService.
        ...
        async def _process_telegram_message(username: str, text: str) -> None:
            try:
                ...
                fingerprint_key = (username, re.sub(r"\s+", " ", text.strip().lower()))
                last_seen = _recent_message_fingerprints.get(fingerprint_key)
                # Prune opportunistically (this dict is small and short-lived
                # by design -- entries older than the window are worthless)
                # instead of a separate scheduled task for it.
                for key in [k for k, ts in _recent_message_fingerprints.items() if (now_check - ts).total_seconds() > _DUPLICATE_MESSAGE_WINDOW_SECONDS]:
                    del _recent_message_fingerprints[key]
                if last_seen is not None and (now_check - last_seen).total_seconds() <= _DUPLICATE_MESSAGE_WINDOW_SECONDS:
                    logger.debug(f"Дубльоване повідомлення з {username} проігноровано.")
                    return
                _recent_message_fingerprints[fingerprint_key] = now_check
                ...
                if is_outcome_report:
                    destroyed_now: list[ThreatMovement] = []
                    for tracked in movements:
                        if tracked.destroyed_at is not None:
                            continue  # already marked destroyed -- nothing to do twice
                        matched = (
                        ...
                        if matched:
                            tracked.status_label = (
                                movement.status_label if movement and movement.status_label else "Знищено"
                            )
                            tracked.destroyed_at = datetime.now()
                            destroyed_now.append(tracked)
                    if destroyed_now:
                        _refresh_movement_views()
                    outcome_label = (movement.status_label if movement and movement.status_label else "знищено/збито").lower()
                    if destroyed_now:
                        logger.history(
                            f"Рух загроз: ціль {outcome_label} -- знято {len(destroyed_now)} поз. з мапи, з {username} "
                            f"({movement.short_description if movement else text})"
                        )
                    else:
                        logger.info(
                            f"Рух загроз: ціль {outcome_label}, але жодної відповідної цілі на мапі не знайдено, з {username}"
                        )
                    return
                ...
                if movement is not None:
                    ...
                    for tracked in movements:
                        if tracked.destroyed_at is None and movement_continues(tracked, movement, now):
                            # Prefer whichever CANDIDATE was updated most
                            # recently -- with more than one active
                            # same-type threat in overlapping oblasts (a
                            # real scenario during a large Shahed wave),
                            # that's the one this new report is most
                            # plausibly continuing, not just whichever
                            # happens to be first in list order.
                            if continued is None or tracked.received_at > continued.received_at:
                                continued = tracked
                    ...
                    if continued is not None:
                        ...
                        # These -- not origin_name/destination_name above --
                        # are what display_origin_name/display_destination_name
                        # (and movement_matches_route_report's later destroy-
                        # message matching) actually prefer; missing this
                        # update left a moved target displaying/matching its
                        # OLD settlement forever after its first update.
                        continued.origin_settlement = movement.origin_settlement or continued.origin_settlement
                        continued.destination_settlement = (
                            movement.destination_settlement or continued.destination_settlement
                        )
                        continued.origin_region = movement.origin_region or continued.origin_region
                        continued.destination_region = movement.destination_region or continued.destination_region
