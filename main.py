    ...
    try:
        ...
        async def _process_telegram_message(username: str, text: str) -> None:
            try:
                ...
                # A "збито/знищено"-style report describes an outcome, not
                # necessarily a brand-new target -- but which of two very
                # different things it means depends on whether THIS message
                # also states its own fresh route/location:
                #   * no route of its own ("Шахед збито над Одещиною") --
                #     purely an update about an existing target, matched
                #     against currently-tracked movements and removed.
                #   * has its own route ("Shahed Boryspil → Voronkiv shot
                #     down") -- a complete, self-contained report; parsed
                #     normally below and added to the map with its outcome
                #     recorded in ``status_label``, not discarded.
                movement = parse_message(text, username, datetime.now())

                if movement is None or not movement.has_any_location:
                    destroyed_report = parse_destroyed_report(text)
                    if destroyed_report is not None:
                        remaining: list[ThreatMovement] = []
                        removed_count = 0
                        for tracked in movements:
                            if movement_matches_destroyed_report(tracked, destroyed_report):
                                removed_count += 1
                            else:
                        ...
                        if removed_count:
                            ...
                            region_names = ", ".join(r.value for r in destroyed_report.regions)
                            logger.info(
                                f"Рух загроз: ціль знищено/збито ({region_names}) -- "
                                f"знято {removed_count} поз. з мапи, з {username}"
                            )
                        return
        ...
        content=ft.Column(
            ...
            controls=[
                ft.Row(
                    spacing=8,
                    controls=[
                        ft.Container(width=10, height=10, border_radius=5, bgcolor=status_color),
...
def _show_district_info_dialog(page: ft.Page, region: Region, district_name: str, state: Optional[RegionState]) -> None:
    ...
    info_rows: list[ft.Control] = [
        ft.Row(
            ...
            controls=[
                ft.Container(width=10, height=10, border_radius=5, bgcolor=status_color),
                ft.Text(f"{status_icon} {status_text}", weight=ft.FontWeight.W_600, color=theme.TEXT_PRIMARY),
            ],
        ),
    ]
...
def _show_movement_dialog(page: ft.Page, movement: ThreatMovement) -> None:
    ...
    info_rows: list[ft.Control] = [
        ft.Row(
            spacing=8,
            controls=[
                ft.Text(f"{movement.threat_type.icon}", size=16),
    ...
    dialog = ft.AlertDialog(
        modal=True,
        title=_dialog_title(f"{movement.threat_type.icon} {movement.threat_type.label_uk}"),
        content=ft.Column(
            tight=True,
            spacing=8,
