...
from app.storage.device_store import DeviceCredentialStore
...
async def main(page: ft.Page) -> None:
    ...
    try:
        ...
        device_store = DeviceCredentialStore(page)
        server_client = ServerClient(logger, device_store)
        ...
        def _on_server_status_changed(status_code: str, message: str) -> None:
            overview_view.set_server_status(status_code, message)
            settings_view.set_server_status(status_code, message)
            settings_view.set_server_details(
                server_client.server_base_url,
                server_client.device_id,
            )
        ...
        async def _start_server_connection() -> None:
            """Launch the fully-automatic server connection lifecycle.

            No IP/Device ID/Token/"Connect" button involved: ServerClient
            connects to the server's fixed, known address over the
            Internet (see app/config.py's get_server_base_url() -- no LAN
            search, works on any network), registers this device if it
            doesn't have saved credentials yet, waits for administrator
            approval if needed, then authenticates and starts polling --
            entirely on its own (see server_client.py's module docstring
            for the full state machine). This call only returns once
            ``server_client.stop()`` is called (app shutdown) or the
            device was explicitly rejected by an administrator.
            """
            device_name = f"Android ({page.platform.value})" if page.platform else "Мобільний пристрій"
            logger.info("Сервер: запуск автоматичного підключення...")
            try:
                await server_client.start(device_name=device_name)
            except Exception as exc:  # noqa: BLE001 - must be visible, never hidden
                logger.error(f"Помилка автоматичного підключення до сервера: {type(exc).__name__}: {exc}")
        ...
        async def _initialize() -> None:
            ...
            try:
                ...
                overview_view.set_server_status("connecting", "Підключення...")
                settings_view.set_server_status("connecting", "Підключення...")

                page.run_task(_tick_loop)
                # Fully automatic, unconditional -- no "enabled" flag, no
                # saved address to check. ServerClient itself handles
                # "do we have credentials yet" (see its module docstring).
                page.run_task(_start_server_connection)
            except Exception as exc:  # noqa: BLE001 - must be visible, never hidden
