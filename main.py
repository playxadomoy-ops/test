    ...
    try:
        ...
        async def _process_telegram_message(username: str, text: str) -> None:
            try:
                ...
                # A "збито/знищено" report describes a target that is
                # gone, not a new one entering the map -- checked BEFORE
                # parse_message() below so a message like "Шахед збито
                # над Одещиною" (which still contains the "шахед"
                # keyword parse_message looks for) doesn't also get
                # added as a brand-new, direction-less movement for a
...
def _show_movement_dialog(page: ft.Page, movement: ThreatMovement) -> None:
    ...
    if movement.group_count > 1:
        info_rows.append(
            ft.Text(
                f"Кількість цілей: ×{movement.group_count}",
                size=12,
                weight=ft.FontWeight.W_600,
                color=theme.TEXT_PRIMARY,
            )
    ...
    info_rows.extend(
        [
            ft.Text(f"Канал: {movement.channel_username}", size=12, color=theme.TEXT_SECONDARY),
            ft.Text(f"Час: {movement.received_at:%H:%M:%S %d.%m.%Y}", size=12, color=theme.TEXT_SECONDARY),
        ]
    )
