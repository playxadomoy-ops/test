...
async def main(page: ft.Page) -> None:
    ...
    try:
        ...
        # module docstring). start() is safe to call unconditionally on
        # every platform: it only ever does real SensorManager work when
        # jnius is actually importable (Android build), and reports the
        # honest "unavailable" diagnostics otherwise -- desktop/web never
        # attempt to touch jnius at all.
        compass_sensor_service = CompassSensorService()
        compass_view.set_compass_diagnostics(compass_sensor_service.start())
