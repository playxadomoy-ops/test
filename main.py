...
async def main(page: ft.Page) -> None:
    ...
    try:
        ...
        #: last processed. Guards against the exact same message being
        #: handled twice (a channel/library redelivering an update, or a
        #: destruction report echoed verbatim by more than one listener
        #: path) -- a short window, since a genuinely new real-world event
        #: reusing identical wording hours apart must still go through.
        _recent_message_fingerprints: dict[tuple[str, str], datetime] = {}
        _DUPLICATE_MESSAGE_WINDOW_SECONDS = 120.0
